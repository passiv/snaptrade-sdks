...
class Configuration(typing.Generic[TAuth]):
    """NOTE:
...
conf = snaptrade_client.Configuration(
    ...
    def __init__(self, host=None,
                 auth: typing.Optional[TAuth] = None,
        ...
        """Client State
        """
        self.auth_mode = getattr(auth, "mode", None) if auth is not None else None
        if auth is not None and self.auth_mode is None:
            raise ClientConfigurationError("auth must include a mode")
        if self.auth_mode == "commercialApiKey":
            if getattr(auth, "consumer_key", None) is not None:
                consumer_key = getattr(auth, "consumer_key")
            if getattr(auth, "client_id", None) is not None:
                client_id = getattr(auth, "client_id")
            if getattr(auth, "consumer_key", None) is not None:
                self.consumer_key = getattr(auth, "consumer_key")
        if self.auth_mode == "personalApiKey":
            if getattr(auth, "consumer_key", None) is not None:
                consumer_key = getattr(auth, "consumer_key")
            if getattr(auth, "client_id", None) is not None:
                client_id = getattr(auth, "client_id")
    ...
    def auth_settings(self):
        ...
        if 'PersonalClientId' in self.api_key or 'PartnerClientId' in self.api_key:
            auth['PersonalClientId'] = {
                'type': 'api_key',
                'in': 'query',
                'key': 'clientId',
                'value': self.get_api_key_with_prefix(
                    'PersonalClientId',
                    alias='PartnerClientId',
                ),
            }
        if 'PersonalSignature' in self.api_key or 'PartnerSignature' in self.api_key:
            auth['PersonalSignature'] = {
                'type': 'api_key',
                'in': 'header',
                'key': 'Signature',
                'value': self.get_api_key_with_prefix(
                    'PersonalSignature',
                    alias='PartnerSignature',
                ),
            }
        if 'PersonalTimestamp' in self.api_key or 'PartnerTimestamp' in self.api_key:
            auth['PersonalTimestamp'] = {
                'type': 'api_key',
                'in': 'query',
                'key': 'timestamp',
                'value': self.get_api_key_with_prefix(
                    'PersonalTimestamp',
                    alias='PartnerTimestamp',
                ),
            }
    ...
    def auth_settings_for_auth_modes(self, auth_modes, fallback_auth_settings=None):
        if not auth_modes:
            return fallback_auth_settings
        if self.auth_mode is None:
            return fallback_auth_settings
        return auth_modes.get(self.auth_mode, [])
