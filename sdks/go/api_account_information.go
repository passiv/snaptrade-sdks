...
type AccountInformationApiGetUserAumPercentileRequest struct {
	ctx context.Context
	ApiService *AccountInformationApiService
	userId string
	userSecret string
...
func (r AccountInformationApiGetUserAumPercentileRequest) Execute() (*UserAumPercentileResponse, *http.Response, error) {
	return r.ApiService.GetUserAumPercentileExecute(r)
}

/*
GetUserAumPercentile Get the user's AUM percentile
...
`data` is `null` — a 200, not an error — when SnapTrade declines to place the user. That happens when the cohort is too small to publish a distribution, or when the user's own holdings are incomplete or stale (for example they hold a disabled connection). A placement computed from a partial view of a user's assets would understate them, so none is returned.
 ...
 @param userId
 @param userSecret
 @return AccountInformationApiGetUserAumPercentileRequest
*/
func (a *AccountInformationApiService) GetUserAumPercentile(userId string, userSecret string) AccountInformationApiGetUserAumPercentileRequest {
	return AccountInformationApiGetUserAumPercentileRequest{
		ApiService: a,
		ctx: a.client.cfg.Context,
		userId: userId,
...
func (a *AccountInformationApiService) GetUserAumPercentileExecute(r AccountInformationApiGetUserAumPercentileRequest) (*UserAumPercentileResponse, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *UserAumPercentileResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "AccountInformationApiService.GetUserAumPercentile")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

    subpath := "/aumPercentile"
	localVarPath := localBasePath + subpath
	if a.client.cfg.Host != "" {
		localVarPath = a.client.cfg.Scheme + "://" + a.client.cfg.Host + subpath
	}
