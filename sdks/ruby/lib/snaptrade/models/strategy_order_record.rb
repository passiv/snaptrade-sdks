  ...
  class StrategyOrderRecord
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SnapTrade::StrategyOrderRecord` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SnapTrade::StrategyOrderRecord`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'closed_quantity')
        self.closed_quantity = attributes[:'closed_quantity']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          closed_quantity == o.closed_quantity &&
    ...
    def hash
      [strategy, status, filled_quantity, open_quantity, closed_quantity, order_type, time_in_force, limit_price, execution_price, time_placed, time_updated].hash
    end
