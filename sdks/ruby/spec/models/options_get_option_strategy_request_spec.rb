=begin
#SnapTrade

#Connect brokerage accounts to your app for live positions and trading

The version of the OpenAPI document: 1.0.0
Contact: api@snaptrade.com
Generated by: https://openapi-generator.tech
OpenAPI Generator version: 6.2.1

=end

require 'spec_helper'
require 'json'
require 'date'

# Unit tests for OpenapiClient::OptionsGetOptionStrategyRequest
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe OpenapiClient::OptionsGetOptionStrategyRequest do
  let(:instance) { OpenapiClient::OptionsGetOptionStrategyRequest.new }

  describe 'test an instance of OptionsGetOptionStrategyRequest' do
    it 'should create an instance of OptionsGetOptionStrategyRequest' do
      expect(instance).to be_instance_of(OpenapiClient::OptionsGetOptionStrategyRequest)
    end
  end
  describe 'test attribute "underlying_symbol_id"' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  describe 'test attribute "legs"' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  describe 'test attribute "strategy_type"' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
      # validator = Petstore::EnumTest::EnumAttributeValidator.new('String', ["CUSTOM"])
      # validator.allowable_values.each do |value|
      #   expect { instance.strategy_type = value }.not_to raise_error
      # end
    end
  end

end
