...
/**
    ...
    public function listConnectionAccountsWithHttpInfo($connection_id, $user_id, $user_secret, string $contentType = self::contentTypes['listConnectionAccounts'][0], \SnapTrade\RequestOptions $requestOptions = null)
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\SnapTrade\Model\ConnectionAccount[]' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\SnapTrade\Model\ConnectionAccount[]' !== 'string') {
                            $content = json_decode($content);
                        }
                    }

                    return [
                        ObjectSerializer::deserialize($content, '\SnapTrade\Model\ConnectionAccount[]', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
                case 401:
            ...
            switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
                        $e->getResponseBody(),
                        '\SnapTrade\Model\ConnectionAccount[]',
                        $e->getResponseHeaders()
                    );
                    $e->setResponseObject($data);
                    break;
